...
def count_flights_within_one_hour(row):
    ...
    filtered_data = cleaned_df[
        (cleaned_df["scheduled_time_departure"] >= start_time)
        & (cleaned_df["scheduled_time_departure"] <= end_time)
        & (cleaned_df["origin"] == airport)
    ]
...
if __name__ == "__main__":
    ...
    print("Computing...")
    results = [
        pool.apply_async(count_flights_within_one_hour, (row,))
    ...
    pd.Series(num_flights).to_csv(
        "/Users/favea/Downloads/swiss-data/num_flights_within_hour.csv"
    )
